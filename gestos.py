...
ROXO_NEON = (255, 0, 255) # Cor Roxa para os olhos
...
nome_janela = "Interface Bio-Tech FB Edition"
cv2.namedWindow(nome_janela, cv2.WINDOW_NORMAL)
cv2.setWindowProperty(nome_janela, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)

# Índices para o mapeamento detalhado dos olhos (Wireframe dos olhos)
...
while True:
    ...
    if res_hands.multi_hand_landmarks:
        for idx, hand_landmarks in enumerate(res_hands.multi_hand_landmarks):
            ...
            # Gesto exclusivo na mão direita para modo rosto
            if info_mao == "Right":
                p4, p8 = hand_landmarks.landmark[4], hand_landmarks.landmark[8]
        ...
        if res_face.multi_face_landmarks:
            for face_landmarks in res_face.multi_face_landmarks:
                ...
                for i in [468, 473]:
                    ...
                    cv2.circle(canvas, (ix, iy), 10, ROXO_NEON, 1, cv2.LINE_AA)
    ...
    if key == ord('r'):
        ...
        if GRAVANDO:
            ...
            filename = f"fb_content_{int(time.time())}.mp4"
    ...
    if GRAVANDO:
        ...
        if int(time.time() * 2) % 2 == 0:
            cv2.circle(canvas, (50, ALTURA_MONITOR - 80), 15, VERMELHO_REC, -1)
        cv2.putText(canvas, "REC", (80, ALTURA_MONITOR - 70), cv2.FONT_HERSHEY_SIMPLEX, 0.8, VERMELHO_REC, 2)
